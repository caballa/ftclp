...
:- module(builtins, [sp_builtin/1, sp_builtin/2, sp_builtin_key/1]).

:- push_prolog_flag(multi_arity_warnings,off).
% sp_builtin(+G)
% Succeeds if G is a goal and a builtin.
sp_builtin(G) :-
	functor(G,F,N),
...
% sp_builtin(+G,-M)
% Succeeds if G is a goal and a builtin and return module name if it
...
sp_builtin(G,Module) :-
	functor(G,F,N),
	is_module_expanded(F,MP,Module),
	library_export(Module,MP,N,_),
	!.
:- pop_prolog_flag(multi_arity_warnings).

% sp_builtin_key(+Key)
% As sp_builtin/1 but Key is F/A
sp_builtin_key(F/N):- 
	builtin_export(_,F,N,_), 
	!.
sp_builtin_key(F/N) :-
...
builtin_export(write,write,2,0) .
builtin_export(write,write,1,0) .
builtin_export(write,writeq,2,0) .
builtin_export(write,writeq,1,0) .
builtin_export(write,print,2,0) .
builtin_export(write,print,1,0) .
builtin_export(write,portray_clause,2,0) .
builtin_export(write,portray_clause,1,0) .
builtin_export(write,numbervars,3,0) .
builtin_export(write,prettyvars,1,0) .
...
%% libraries
library_export(random, random,3,0) .
library_export(lists , nth   ,3,0) .
library_export(lists , length,2,0) .
