...
tclp_expansion(end_of_file, _ ):- !.
	%delete_all_counters.
%----------------------------------------------------------------------%
% Expand directives
%----------------------------------------------------------------------%
tclp_expansion((:- tabled(Pred))         , tabled(Pred)).
%	format("tabled(~q).\n",[Pred]).
tclp_expansion((:- no_cache(Pred))       , no_cache(Pred)).
%	format("no_cache(~q).\n",[Pred]).
tclp_expansion((:- discriminants(Pred))  , discriminants(Pred)).
%	format("discriminants(~q).\n",[Pred]).
%% Deprecated mode/1
tclp_expansion((:- mode(Pred))      , mode(Pred)).
%	format("mode(~q).\n",[Pred]).
tclp_expansion((:- Dir)             , (:- Dir)).
%	format(":- ~q \n",[Dir]).
%----------------------------------------------------------------------%
% Expand clauses
%----------------------------------------------------------------------%
tclp_expansion((Head :- Body)      , rule(Head,K,List)):-
	functor_to_atom(Head,Name),
	incr_counter(Name, K),
          flatten_goals(Body,List,[]).
%	print_rule(Head, K, List).
tclp_expansion((Head)               , rule(Head,K,[])):-
	functor_to_atom(Head,Name),
	incr_counter(Name, K).
%	print_rule(Head, K, []).
flatten_goals((G1 , G2)) --> !,
