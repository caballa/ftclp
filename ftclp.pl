...
:- module(ftclp, [ ftclp/5, main/1, get_map_node_to_unique_id/4, is_fake_false_id/1]).
...
:- use_module('frontend/ftclp_loadprogram').
:- use_module('frontend/ftclp_tr').
...
main(ArgVs):-
	...
	; 
	  ( var(Depth) -> 
   	    ftclp(InputFile,Entry,SolverName,0,Options)
	  ;
	    ftclp(InputFile,Entry,SolverName,Depth,Options)  
	  )
	),
	!.
main(_):- !.
...
%---------------------------------------------------------------------%
%  From the program transformation done by ftclp_tr
%---------------------------------------------------------------------%
:- multifile rule/3.
:- multifile builtin/3.
...
add_ftclp_in_ciao_libpaths :-
          ftclp_path(FTCLP_Root),
          asserta_fact(library_directory(FTCLP_Root)),
          asserta_fact(file_search_path(ftclp, FTCLP_Root)).
        
%---------------------------------------------------------------%
% ftclp(+atm,+term,+atm,+num,+list(atm))
%---------------------------------------------------------------%
ftclp(InFile, Goal, SolverName, MaxDepth, Opts):-
	cleanup,
	init_timers,
	...
	% further calls to ftclp/2.
	unload_program(InFile2),
	!.
ftclp(_,_,_,_,_):- !. % Here if input filename is not found.
